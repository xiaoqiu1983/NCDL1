# 使用theano作为backend
#产生所有的信息组合，不在[0, q-1],而在[-1, q-2]上，和matlab里通过唯一好用rs码产生的MDS码字相对应，那个码字就是在[-1, q-2]
...
#Parameters
k = 5                       # number of information symbols
N = 15                     # code length
system = 16
...
def eight_add(aa):
    ...
    for i in range(max(len(m_list), len(n_list))):
        ...
        if plus >= (system-1):  # 本位大于8，本位存本位和-8，并向前进一
            result[i] = str(plus - (system-1))#本来是system，比如8进制就是8，但是这里用(system-1)，主要是为了配合[-1, q-2]
            flag = True
        else:
            result[i] = str(plus)
            flag = False
